---
title: "Primates Costa Rica"
output: html_document
---

```{r carga de library, message=FALSE}
library(dplyr)
library(sf)
library(DT)
library(plotly)
...
```{r carga de datos, message=FALSE, echo=TRUE, warning=FALSE}
primates_cr <-
  st_read(
...
```{r capas, message=FALSE, echo=TRUE, warning=FALSE}
...
```{r tabla, message=FALSE, echo=TRUE, warning=FALSE}
primates_cr %>%
  st_drop_geometry() %>%
...
```{r variables, message=FALSE, echo=TRUE, warning=FALSE }
...
```{r filtros, message=FALSE, echo=TRUE, warning=FALSE}
...
```
## Mapa raster
```{r mapa, message=FALSE, echo=TRUE, warning=FALSE}
 ## mapa de especies
  primates_cr %>%
    leaflet() %>%
